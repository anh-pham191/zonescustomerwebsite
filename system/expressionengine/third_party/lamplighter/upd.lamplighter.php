...
class Lamplighter_upd
{
	...
	public function install()
	{
		...
		ee()->dbforge->add_field($licensing);
		ee()->dbforge->add_key('id', TRUE);
		ee()->dbforge->create_table('lamplighter_license', TRUE);
		...
		if ( ee()->db->field_exists('csrf_exempt', 'actions')) {
			$data['csrf_exempt'] = 1;
		}
	...
	{
		ee()->load->dbforge();
		ee()->dbforge->drop_table('lamplighter_license');
		...
		ee()->db->where('module_name', 'Lamplighter')
					 ->delete('modules');

		ee()->db->where('class', 'Lamplighter')
					 ->delete('actions');
	...
	public function update($current = '')
	{
		...
		if (version_compare('1.2.1', $current) == 1) {
			...
			to 1.2.3
			*/
			if ( ! ee()->db->field_exists('lamplighter_site_id', 'lamplighter_license') ) {

				ee()->dbforge->modify_column('lamplighter_license',
					array(
						'site_id' => array(
				...
				ee()->dbforge->add_column('lamplighter_license',
					array(
						'site_id' => array(
							'type' => 'INT'
						)
				...
				ee()->db->set('site_id', ee()->config->item('site_id'))
