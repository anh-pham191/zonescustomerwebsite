...
class Devotee_library
{
	...
	public function __construct()
	{
		...
		if (ee()->config->item('lamplighter_cachepath')) {
			$this->cache_path = ee()->config->item('lamplighter_cachepath');
		} else if (ee()->config->item('devotee_monitor_cachepath')) {
			$this->cache_path = ee()->config->item('devotee_monitor_cachepath');
		} else {
			$this->cache_path = ee()->lamplighter_library->ee3() ? SYSPATH.'user/cache/lamplighter/' : APPPATH . 'cache/lamplighter/';
		}
		...
		if (is_array(ee()->config->item('lamplighter_ignored_addons'))) {
			$this->ignored_addons = array_merge($ignored_addons, ee()->config->item('lamplighter_ignored_addons'));
		} else if (is_array(ee()->config->item('devotee_monitor_ignored_addons'))) {
			$this->ignored_addons = array_merge($ignored_addons, ee()->config->item('devotee_monitor_ignored_addons'));
		} else {
	...
	{
		ee()->load->helper(array('file', 'language'));
		...
		elseif(ee()->input->get('C') == 'addons_plugins')
		{
			return ee()->load->view('acc_error', array(
		...
		else
		{
			...
			if ( ee()->lamplighter_library->ee3() ) {
				ee()->legacy_api->instantiate('channel_fields');
			} else {
				ee()->api->instantiate('channel_fields');
			}
			// Get fieldtypes because the add-ons library doesn't give all the info
			$fieldtypes = ee()->api_channel_fields->fetch_all_fieldtypes();

			// Set third-party add-ons
			$third_party = array_intersect_key(ee()->addons->_packages, $map);
			...
			$installed = array(
				...
				'accessories' => ee()->addons->get_installed('accessories')
			);
			...
			foreach($third_party as $package => $types)
			{
				...
				if(array_key_exists($package, $installed['modules']))
				{
					...
					ee()->lang->loadfile(( ! isset($this->lang_overrides[$package])) ? $package : $this->lang_overrides[$package]);
		...
		if ( $return_view ) {
			...
			// Return the view
			return ee()->load->view('acc_accessory', array(
	...
	public function get_updates()
	{
		$data = array(
			'data'    => $this->_addons,
			'site'    => md5( ee()->config->item('site_label') ),
	...
	{
		// Hidden add-ons
		$hidden_addon_query = ee()->db->select('package')
	...
	{
		return method_exists(ee()->localize, 'string_to_timestamp') ? ee()->localize->string_to_timestamp(date('Y-m-d H:i:s',filemtime($this->cache_file)), true) : filemtime($this->cache_file);
	}
	...
	function get_plugins()
	{
		...
		if ( ee()->lamplighter_library->ee3() ) {
			$plugin_results = ee()->db->get('plugins')->result_array();
	...
	{
		ee()->db->insert('lamplighter_hidden_addons', array(
			'member_id' => $member_id,
			'package'   => $package
		));
	}
	...
	{
		ee()->db->where('package', $package)->where('member_id', $member_id)->delete('lamplighter_hidden_addons');
	}
}
