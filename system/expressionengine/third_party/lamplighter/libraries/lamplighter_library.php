...
class Lamplighter_library
{
	...
	public function __construct()
	{
		...
		// Override the Lamplighter URL for testing.
		if ( ee()->config->item('lamplighter_test_url') ) {
			$this->lamplighter_url = ee()->config->item('lamplighter_test_url');
		}
	...
	public function store_token_data($site_token=null)
	{
		...
		if (isset($key[1])) {
			...
			$this->api_key = $key[1];
			ee()->db->insert('lamplighter_license', array(
				'key' => $this->api_key,
	...
	{
		$action_id = ee()->cp->fetch_action_id('Lamplighter', 'api_request');
	...
	{
		if ( ee()->db->table_exists('lamplighter_license') ) {
			$query = ee()->db->select('lamplighter_site_id, key')
	...
	{
		ee()->db->delete('lamplighter_license', array('site_id' => $this->site_id));
	}
	...
	protected function _php_info()
	{
        ...
        $tables = ee()->db->query('SHOW TABLE STATUS')->result_array();
		...
		$mysql_info = ee()->db->query('SELECT VERSION() AS version');
	...
	protected function _api_addons()
	{
		...
		$addons = json_encode(ee()->devotee_library->get_addons(FALSE, FALSE, TRUE));
