...
class Lamplighter_mcp
{
	...
	*/
	...
	public function __construct()
	{
		...
		$this->ee3 = ee()->lamplighter_library->ee3();
		...
		if ( !empty(ee()->lamplighter_library->api_key) ) {
			if ( $this->ee3 ) {
	...
	public function index()
	{
		...
		if ( ! $this->ee3 ) {
			ee()->functions->redirect($this->_token_url);
		}
		// For EE3, show the add-on updates table similar to the one from the EE2 accessory.
		ee()->view->cp_page_title = lang('lamplighter_module_name');
		$view_data = array(
			'updates' => ee()->devotee_library->get_addons(false, false),
			'last_check' => ee()->devotee_library->last_check_date(),
			'hidden_addons' => ee()->devotee_library->fetch_hidden_addons(),
			'cp' => ee()->cp,
			'base_url' => $this->_base_url,
			'update_url' => $this->update_url,
			'hide_addon_url' => $this->hide_addon_url,
			'unhide_addon_url' => $this->unhide_addon_url,
			'show_hidden' => ee()->input->get('show_hidden', true) == 'y' ? true : false
		);
		ee()->cp->add_to_foot('<link rel="stylesheet" href="' . $this->theme_url . 'styles/cp.css?v=' . LAMPLIGHTER_VERSION . '" />');
		ee()->cp->add_to_foot('<script type="text/javascript" src="' . $this->theme_url . 'scripts/cp.js?v=' . LAMPLIGHTER_VERSION . '"></script>');
	...
	{
		ee()->view->cp_page_title = lang('token');
		$view_data = array(
			'api_key' => ee()->lamplighter_library->getSiteToken(),
	...
	public function refresh()
	{
		...
		$response = ee()->lamplighter_library->api_request($api_endpoint);
		if ( $response['status'] == 'success' ) {
			$this->success_alert($response['message']);
		} else {
			ee()->lamplighter_library->purge_token_data();
	...
	{
		$site_token = ee()->input->post('api_key');
		$response = ee()->lamplighter_library->store_token_data($site_token);
		if ( $response['status'] == 'success' ) {
			$this->success_alert($response['message']);
		} else {
			ee()->lamplighter_library->purge_token_data();
	...
	{
		$response = ee()->lamplighter_library->unregister_action_id();
	...
	public function unhide_addon()
	{
		...
		$package_name = ee()->input->get('package');
		// Delete hidden add-on setting for user
		ee()->devotee_library->unhide_addon($package_name, ee()->session->userdata('member_id'));
		return ee()->functions->redirect($this->_base_url);
	}
	...
	{
		ee()->devotee_library->clear_cache();
		return ee()->functions->redirect($this->_base_url);
	}
