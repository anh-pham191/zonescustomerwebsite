...
class Lamplighter_acc
{
	...
	public function __construct()
	{
		...
		ee()->load->library('devotee_library');

		// Set theme URL
		$this->theme_url = defined('URL_THIRD_THEMES')
			? URL_THIRD_THEMES . '/lamplighter/'
			: ee()->config->item('theme_folder_url') . 'third_party/lamplighter/';
	...
	public function uninstall()
	{
		...
		ee()->dbforge->drop_table('lamplighter_hidden_addons');
	...
	public function set_sections()
	{
		...
		ee()->cp->add_to_foot('<link rel="stylesheet" href="' . $this->theme_url . 'styles/accessory.css?v=' . $this->version . '" />');
		ee()->cp->add_to_foot('<script type="text/javascript" src="' . $this->theme_url . 'scripts/accessory.js?v=' . $this->version . '"></script>');
	}
	...
	protected function _init()
	{
		...
		return ee()->load->view('acc_init', array(
			'link' => $link,
			'cp'   => ee()->cp,
		), TRUE);
	}
	...
	{
		if ( ! ee()->db->table_exists('lamplighter_hidden_addons') ) {
			ee()->dbforge->add_field('id int(10) unsigned NOT NULL AUTO_INCREMENT');
			ee()->dbforge->add_field('member_id int(10) unsigned NOT NULL');
			ee()->dbforge->add_field('package varchar(100) NOT NULL');
			ee()->dbforge->add_key('id', TRUE);
			ee()->dbforge->add_key('member_id');
			ee()->dbforge->create_table('lamplighter_hidden_addons', TRUE);
		}
	}
	...
	public function process_refresh($delete_cache = TRUE, $display_hidden_addons = FALSE)
	{
		if(AJAX_REQUEST)
		{
			...
			if($delete_cache)
			{
				ee()->functions->delete_directory(APPPATH . 'cache/lamplighter');
			}

			// Output HTML from the view
			echo ee()->devotee_library->get_addons($display_hidden_addons);
			exit;
		}
	}
	...
	public function process_hide_addon()
	{
		if(AJAX_REQUEST)
		{
			...
			ee()->devotee_library->hide_addon(ee()->input->get('package'),ee()->session->userdata('member_id'));
			// Refresh view
			$this->process_refresh(FALSE);
		}
	}
	...
	public function process_unhide_addon()
	{
		if(AJAX_REQUEST)
		{
			...
			ee()->devotee_library->unhide_addon(ee()->input->get('package'),ee()->session->userdata('member_id'));
			// Refresh view
			$this->process_refresh(FALSE);
		}
	}
	...
	public function process_debug_info()
	{
		...
		$vars = array(
			...
			'db_driver'		     => ee()->db->dbdriver,
			'updates'		     => ee()->devotee_library->get_addons(TRUE, FALSE),
			'browser'		     => ee()->agent->browser().' '.ee()->agent->version(),
			'cookie_domain'	     => ee()->config->item('cookie_domain'),
			'cookie_path'        => ee()->config->item('cookie_path'),
			'user_session_type'  => ee()->config->item('user_session_type'),
			'admin_session_type' => ee()->config->item('admin_session_type'),
			'cp_cookie_domain'	 => ee()->config->item('cp_cookie_domain'),
			'cp_cookie_path'     => ee()->config->item('cp_cookie_path'),
			'cp_session_ttl'     => ee()->config->item('cp_session_ttl'),
			'server_software'    => $server_software
		);

		header('Content-Type : text/plain');
		exit( ee()->load->view('acc_debug_info', $vars, TRUE) );
	}
