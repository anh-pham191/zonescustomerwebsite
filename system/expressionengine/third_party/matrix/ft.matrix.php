...
class Matrix_ft extends \matrix\Eeharbor_ft
{
     ...
     *
     *   ** NOTE ** This will not work in ExpressionEngine 4. (channel_data query)
     *       I can not imagine this will ever get run in EE4, but EE can perform
     *       mystical witchcraft at times. Be forewarned.
     *
    ...
    public function post_save($data)
    {
        ...
        $this->_updateChannelData($entry_id, $data);
    ...
    private function _updateChannelData($entry_id, $data)
    {
        ...
        {
            $channelEntry = ee('Model')->get('ChannelEntry')
                                        ->filter('entry_id', '==', $entry_id)
                                        ->first();

            $channelEntry->fill($data);
            $channelEntry->save();
        }
    }
    /**
