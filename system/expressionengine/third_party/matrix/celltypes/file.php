    ...
    public function display_cell($data)
    {
        ...
        if(! $eeharbor->is_ee2())
        {
            ...
            if (REQ == 'CP') {
                ...
                $ee3_data = $filePicker
                        ->setDirectories($allowed_filedir)
                        ->getLink("Add File")
                ...
                $ee3_data = '<input type="file" name="' . $this->cell_name . '" class="file-chooser" /><br />';
            }
...
// exit;
    ...
    public function save_cell($data)
    {
        ...
        // Fix the $field_name if it's actually in the form 'field_id_x'
        $has_field_id_for_name = (strncmp($field_name, 'field_id_', 9) === 0);
        if ($has_field_id_for_name && (REQ != 'CP')) {
            $field_id = (int) substr($field_name, 9);
            $field_name = $this->_get_field_name_by_id($field_id);
        }
     ...
     */
    private function _get_field_name_by_id($field_id)
    {
        ...
        if ($query->num_rows()) {
            return $query->first_row()->field_name;
        }
    }
