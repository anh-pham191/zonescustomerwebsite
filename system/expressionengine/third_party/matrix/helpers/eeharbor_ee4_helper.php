...
require_once PATH_THIRD.'matrix/helpers/eeharbor_abstracted.php';
...
class EEHelper extends \matrix\EEHarbor_abstracted {
	...
	public function cache($mode, $key = false, $data = false, $persistent = true) {
		// Returns EE's native cache function for EE4.
		switch($mode) {
			case 'get':
	 ...
	 * @param  array  $extra_parameters Name of EE4 alert functions to call in addition to the default ones. (does nothing in EE2) ex. ['cannotClose']
	 */
	...
	public function getAddonThemesDir() {
		$theme_folder_url = (defined('URL_THIRD_THEMES') ? URL_THIRD_THEMES : ee()->config->slash_item('theme_folder_url').'user/') . $this->_module . '/';
		return $theme_folder_url;
	}

	/**
	 * Overwrite any native EE Classes.
	 * EE4 uses EE's set() method.
	 *
	...
	private function initiateSettings()
	{
		...
		$addonSettings = require PATH_THIRD.'matrix/addon.setup.php';
