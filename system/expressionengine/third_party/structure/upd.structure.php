...
class Structure_upd {
	...
	private function create_table_structure()
	{
		...
		if ( ! ee()->db->table_exists('structure'))
		{
			$fields = array(
				...
				'structure_url_title' => array('type' => 'varchar', 'constraint' => '255', 'null' => TRUE),
	...
	{
		if ($current == '' || $current == $this->version || ee()->input->get('M') == "do_update")
			return FALSE;
		...
		if (version_compare($current, '4.2.1', "<")) {
			// there was an issue where site_pages type wasn't changed to be
			$this->confirm_site_pages_type();
		}
	...
	function populate_listings()
	{
		...
		foreach($site_pages['uris'] as $entry_id => $uri) {
			...
			if ($entry_id && ! $node) {
				...
				if(is_array($site_pages['templates'][$entry_id])) $template_id = $site_pages['templates'][$entry_id][0];
				else $template_id = $site_pages['templates'][$entry_id];
				...
				foreach($data as $value)
				{
					if(is_array($value) || is_null($value))
						continue 2; // This will skip the current iteration of the foreach loop, so we dont try to insert
				}
