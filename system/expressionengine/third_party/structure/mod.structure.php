...
class Structure extends Channel
{
	...
	public function __construct()
	{
		...
		if(version_compare(APP_VER, '2.11.0', '<'))
		{
			parent::Channel();
		} else {
			parent::__construct();
	...
	public function nav()
	{
		...
		if ($start_from != '/') {
			...
			} else if($strict_start_from !== FALSE) {
				return '';
			}
		}
	...
	//{
	//	$cat = ee()->TMPL->fetch_param('start_from','');
	...
	public function nav_main()
	{
		...
		$css_id = strtolower(ee()->TMPL->fetch_param('css_id', 'nav'));
		...
		$add_unique_id = ee()->TMPL->fetch_param('add_unique_id', 'no');

		$add_unique_to = ee()->TMPL->fetch_param('add_unique_to', ee()->TMPL->fetch_param('nav_class', 'li'));

		$current_class = ee()->TMPL->fetch_param('current_class', 'here');

		$include_ul = ee()->TMPL->fetch_param('include_ul', 'yes');

		$add_span = ee()->TMPL->fetch_param('add_span', 'no');

		$css_class = ee()->TMPL->fetch_param('css_class', '');

		// exclude and include entry_ids
		$exclude = explode("|", ee()->TMPL->fetch_param('exclude'));
		$include = (ee()->TMPL->fetch_param('include')) ? " AND `node`.`entry_id` IN(".str_replace("|", ",", ee()->TMPL->fetch_param('include')).") " : "";
		...
		if (count($result->num_rows()) > 0)
		{
			...
			foreach ($result->result_array() as $entry_data)
			{
				...
				$the_uri = parse_url(ee()->db->escape_str($site_url));
				$root_uri = $the_uri['path'];
				$index_uri = $site_url;
				$home = trim(ee()->functions->fetch_site_index(0, 0), '/');
			...
			$css_class = (ee()->TMPL->fetch_param('css_class')) ? " class=\"" . ee()->TMPL->fetch_param('css_class') . "\"" : "";
	...
	public function nav_sub()
	{
		...
		$include_ul = (ee()->TMPL->fetch_param('include_ul')) ? ee()->TMPL->fetch_param('include_ul') : 'yes';
		$add_span = (ee()->TMPL->fetch_param('add_span')) ? ee()->TMPL->fetch_param('add_span') : 'no';

		$css_id = ee()->TMPL->fetch_param('css_id') ? ee()->TMPL->fetch_param('css_id') : '';
		...
		// Current CSS class
		$current_class = (ee()->TMPL->fetch_param('current_class')) ? ee()->TMPL->fetch_param('current_class') : 'here';

		// Last CSS Class
		$css_last_class = ee()->TMPL->fetch_param('css_last_class', 'last');

		// show_level_classes
		$show_level_classes = strtolower(ee()->TMPL->fetch_param('show_level_classes', 'yes'));

		// show_overview_link
		$show_overview_link = strtolower(ee()->TMPL->fetch_param('show_overview_link', 'no'));

		// Custom View Text
		$overview_text_title = strtolower(ee()->TMPL->fetch_param('show_overview_link', 'no'));

		// Rename Overview parameter
		$overview_link_text = ee()->TMPL->fetch_param('rename_overview', 'Overview');

		// Add CSS ids based on the URI to all <li> elements
		$add_unique_id = strtolower(ee()->TMPL->fetch_param('add_unique_id', 'no'));

		// Add CSS ids based on the entry_id to all <li> elements
		$add_unique_entry_id = strtolower(ee()->TMPL->fetch_param('add_unique_entry_id', 'no'));

		$wrap_start = ee()->TMPL->fetch_param('wrap_start', 'no');

		$wrap_end = ee()->TMPL->fetch_param('wrap_end', 'no');

		// Hide the entire sub nav if there are no children of the current page
		$hide_if_no_children = ee()->TMPL->fetch_param('hide_if_no_children', 'no');
		$initiate_children_hiding = 'no';

		// depth to limit, never expand past this depth
		$depth_limit = ee()->TMPL->fetch_param('limit_depth', FALSE);

		// depth to show, show all of tree up to specified depth
		$show_n_deep = ee()->TMPL->fetch_param('show_depth');
	...
	public function nav_full()
	{
		...
		$add_unique_id = ee()->TMPL->fetch_param('add_unique_id');
		...
		{
			$page_uri = $site_pages['uris'][$page['entry_id']];
			$uri = ee()->functions->fetch_current_uri();
	...
	public function breadcrumb()
	{
		...
		// if (array_key_exists('structure_pagination_segment', ee()->config->_global_vars))
...
// exit;
	...
	public function set_data($data, $cache_bust=false)
	{
		...
		if ($channel_type == 'page')
		{
			...
			// existing node
			$changed = true;// $this->has_changed($node, $data);

			if ($changed)
			{
				// Stay connected - only if it's EE2 or less.
				if($this->foundation->getEEVersion() <= 2) ee()->db->reconnect();
	...
	*/
	...
	public function set_listings($data)
	{
		...
		// ee()->db->query(ee()->db->update_string('exp_sites', array('site_pages' => base64_encode(serialize($site_pages))), "site_id = $site_id"));
		// $this->sql->update_root_node();
	}
	...
	*/
	...
	public function delete_data_by_channel($channel_id)
	{
		...
		// add_structure_nav_revision($this->site_id, 'Pre deleting data by channel');
		...
		// add structure nav history before deleting data by channel
		add_structure_nav_revision($this->site_id, 'Post deleting data by channel');
