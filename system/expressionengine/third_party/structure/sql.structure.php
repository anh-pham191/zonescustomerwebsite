...
class Sql_structure
{
	...
	function get_settings()
	{
		...
		if (is_array($settings) || !$this->foundation->module_installed('structure')) {
			return $settings;
		}
	 ...
	 * @return array
	 */
	function get_selective_data($site_id, $current_id, $branch_entry_id, $mode, $show_depth, $max_depth, $status, $include, $exclude, $show_overview, $rename_overview, $show_expired, $show_future, $override_hidden_state="no",$recursive_overview="no",$include_site_url="yes")
	{
	...
	function generate_nav($selective_data, $current_id, $entry_id, $mode, $show_overview, $rename_overview,$override_hidden_state="no", $recursive_overview="no", $level=1)
	{
		...
		for ($i=0; $i < $tree_count; $i++)
		{
			...
			if ($show_overview) $classes = str_replace("first", "", $classes);
	...
	private function _get_sql_fields($custom_titles, $title_fields)
	{
		...
		ee()->load->library('api');
		$this->foundation->instantiate('channel_fields');
	...
	{
		// if EE is lower than V4, run the old code
		if($this->foundation->ver_lt(4))
		{
			$page_titles = $this->_get_page_titles_ee23($sql_fields, $include_listings);
		}
		// run new function for getting the page titles.
		else
		{
			$page_titles = $this->_get_page_titles_ee4($sql_fields, $include_listings);
		}
	...
	private function _get_page_titles_ee23($sql_fields, $include_listings)
	{
		// we need the channel_id's of the structure entries
		$structure_channel_ids_sql = $this->_get_structure_channel_ids($include_listings, $as_string = true);

		// We are building a query that will get the data we need. This is what doesnt work in EE4.
		$select_statement = array();
		foreach ($sql_fields as $channel_id => $field)
		{
			// create select statement. We want to get each custom field and add it to this array.
		...
		$sql = "SELECT c_data.channel_id, c_data.entry_id, ". implode(',', $select_statement) .", c_titles.title
				...
				WHERE c_data.channel_id IN ($structure_channel_ids_sql)
	...
	private function _get_page_titles_ee4($sql_fields, $include_listings)
	{
		// we need the channel_id's of the structure entries
		...
		foreach ($channelEntries as $channelEntry)
		{
			...
			$custom_field_exists = (array_key_exists($channelEntry->channel_id, $sql_fields)	// the channel_id is in the array
			...
			if($custom_field_exists)
			{
				...
				$custom_field_names = $channelEntry->getCustomFieldNames();
				$custom_field_data = $channelEntry->getCustomField($field_id)->getData();

				// we should get the custom field, if it is available on the channel entry, and if it has data in it
				$should_get_field = in_array($field_id, $custom_field_names)
									&& !empty($custom_field_data);
			}

			// Set the page titles array accordingly
			if($should_get_field)
			{
				$page_titles[$channelEntry->entry_id] = $channelEntry->getCustomField($field_id)->getData();
			}
			else
			{
...
// TODO
	...
	function get_listing_entry_ids()
	{
		...
		$listing_ids = $this->foundation->cache('get', 'listing_ids');
		$listing_ids_empty = $this->foundation->cache('get', 'listing_ids_empty');
		...
		if(empty($listing_ids_empty) && (!is_array($listing_ids) || empty($listing_ids)))
		{
			...
			if ($result->num_rows > 0)
			{
				...
				// Cache::set(array('listing_ids'), $listing_ids);
				$this->foundation->cache('set', 'listing_ids', $listing_ids);
				$this->foundation->cache('set', 'listing_ids_empty', 'false');
			} else {
				$this->foundation->cache('set', 'listing_ids_empty', 'true');
			}
		}
	...
	{
		$module_id_query = $this->foundation->cache('get', 'module_id_query');

		if(! $module_id_query)
		{
			$module_id_query = ee()->db->query("SELECT module_id FROM exp_modules WHERE module_name = 'Structure'")->result();

			$this->foundation->cache('set', 'module_id_query', $module_id_query);
		}
	...
	*/
	...
	function get_member_groups()
	{
		...
		if($this->foundation->getEEVersion() == 2) {
	...
	function update_integrity_data()
	{
		$site_pages = $this->get_site_pages(true);
	...
	function cleanup_check()
	{
		...
		if(count($site_pages_uri_duplicates) > 0) {
			...
			foreach($site_pages_uri_duplicates as $entry_id => $uri) {
				...
				$vals['site_pages_uri_duplicates'][$entry_id]['ee_url'] = $this->foundation->cpURL('publish', 'edit', array('entry_id'=>$entry_id));
			}
		}
		...
		foreach($structure_entries as $structure_entry) {
			...
			if(!empty($ee_exists) && $ee_exists == 1) {
				...
				$vals['orphaned_entries'][$structure_entry['entry_id']]['ee_url'] = $this->foundation->cpURL('publish', 'edit', array('entry_id'=>$structure_entry['entry_id']));
			} else {
				$eeOrphans++;
			}
			...
			if(in_array($structure_entry['entry_id'], $site_pages_entry_ids)) {
				...
				if(!empty($site_pages_url)) {
					...
					if(strpos($check_url, '/')) $check_url = substr($check_url, strrpos($check_url, '/')+1);

					if(empty($check_url) && $site_pages_url == '/') $check_url = '/';
				}
				...
				if($structure_entry['structure_url_title'] != $check_url) {
					...
					$vals['mismatch_url_entries'][$structure_entry['entry_id']]['ee_url'] = $this->foundation->cpURL('publish', 'edit', array('entry_id'=>$structure_entry['entry_id']));
				}
				...
				if(!$this->is_valid_template($structure_entry['template_id'])) {
					...
					$vals['template_id_errors'][$structure_entry['entry_id']]['ee_url'] = $this->foundation->cpURL('publish', 'edit', array('entry_id'=>$structure_entry['entry_id']));
				}
			...
			if(!empty($structure_entry['listing_cid']) && !in_array($structure_entry['listing_cid'], $checkedListingChannels)) {
				...
				foreach($listing_entries as $listing_entry) {
					...
					if(!empty($listing_exists) && $listing_exists == 1) {
						...
						if($listingSpExists) {
							...
							if($listing_data['uri'] != $check_url) {
								...
								$vals['mismatch_url_entries'][$listing_entry['entry_id']]['ee_url'] = $this->foundation->cpURL('publish', 'edit', array('entry_id'=>$listing_entry['entry_id']));
							}
							...
							if(!$this->is_valid_template($listing_data['template_id'])) {
								...
								$vals['template_id_errors'][$listing_entry['entry_id']]['ee_url'] = $this->foundation->cpURL('publish', 'edit', array('entry_id'=>$listing_entry['entry_id']));
							}
						}
					} else {
					...
					if(!$listingSpExists || !$listingStructureExists) {
						...
						$vals['orphaned_entries'][$listing_entry['entry_id']]['ee_url'] = $this->foundation->cpURL('publish', 'edit', array('entry_id'=>$listing_entry['entry_id']));
					}
				}
			}
		...
		if($structureOrphans > 0) {
			...
			foreach($site_pages_remaining as $entry_id => $url_title) {
				...
				$vals['orphaned_entries'][$entry_id]['ee_url'] = $this->foundation->cpURL('publish', 'edit', array('entry_id'=>$entry_id));
	 ...
	 **/
	function cleanup($mode='site_pages')
	{
	...
	function generate_site_pages_array()
	{
		$debug = false;
		...
		foreach($structure_entries as $structure_entry) {
			...
			if($debug) echo '<strong>', $entry_id, ' ', $structure_url_title, '</strong><br />';

			if(!empty($structure_entry['parent_id']) && $structure_entry['parent_id'] != $entry_id) {
				if($debug) echo 'P: ', $structure_entry['parent_id'], '<br />';
				...
				if(!empty($site_pages['uris'][$structure_entry['parent_id']])) {
					...
					if($debug) echo 'Existing Parent: ', $site_pages['uris'][$structure_entry['parent_id']], '<br />';
				} else {
					...
					if($debug) echo 'Gen Parent: ', $parent_structure_url_title, '<br />';
				}
			}

			$structure_url_title = strtolower($structure_url_title);
			if($debug) echo 'Final Title: ', $structure_url_title, '<br />';

			if(empty($structure_url_title)) $structure_url_title = '/';
			else {
				if(substr($structure_url_title, 0, 1) != '/') $structure_url_title = '/'.$structure_url_title;
			...
			if(empty($site_pages['templates'][$entry_id])) $site_pages['templates'][$entry_id] = $structure_entry['template_id'];
			...
			if(!empty($structure_entry['listing_cid'])) {
				...
				foreach($listing_entries as $listing_entry) {
					...
					if(empty($site_pages['templates'][$listing_entry_id])) $site_pages['templates'][$listing_entry_id] = $structure_entry['template_id'];
				}
			}
			if($debug) echo '<br />';
		}
		if($debug) die('END generate_site_pages_array');

		return $site_pages;
	}

	function retrieve_structure_url_title($entry_id)
	{
	...
	// DEPRECATED
	function restore_site_pages_from_structure()
	{
		$site_pages = $this->get_site_pages(true);
	...
	// Deprecated in favor of generate_site_pages_array()
	...
	// 					// {
	// 					// 	$new_array[$entry_id] .= $data[$data[$data[$data[$row['parent_id']]['parent_id']]['parent_id']]['parent_id']]['url_title'].'/';
	// 					// }
	// 					$uris[$entry_id] .= $data[$data[$data[$row['parent_id']]['parent_id']]['parent_id']]['url_title'].'/';
	// 				}
	// 				$uris[$entry_id] .= $data[$data[$row['parent_id']]['parent_id']]['url_title'].'/';
	// 			}
	// 			$uris[$entry_id] .= $data[$row['parent_id']]['url_title'].'/';
	// 		}
	// 		$uris[$entry_id] .= $row['url_title'].'/';
	// 	}
...
static function build_from_results($results)
{
	...
	foreach ($results as $row)
	{
		...
		if(!in_array($row['parent_id'], $parent_ids)) continue;
