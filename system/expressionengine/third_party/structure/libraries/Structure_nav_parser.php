...
class Structure_core_nav_parser
{
    ...
    protected function add_entry_vars_ee4()
    {
        ...
        if(!empty($_GET['testing']) && $_GET['testing'] == 1) {
            // Loop through the channel entries and parse the variables
            foreach ($channelEntries as $channelEntry)
            {
                 // if this row is set, then parse it
                 if (isset($this->rows_by_entry[$channelEntry->entry_id]))
                 {
                     ...
                     // Assign the value of each custom field
                     foreach ($channelEntry->getCustomFieldNames() as $field_name)
                     {
                         if (preg_match('/^field_(id|ft|dt)_/', $field_name))
                         {
                             $field = $prefix.$channelEntry->getCustomField($field_name)->getItem('field_name');
                         }
                         else
                         {
                             $field = $prefix.$field_name;
                         }

                         $variable_row[$field] = $channelEntry->getCustomField($field_name)->getData();

                         // if(substr($field_name, 0, 6) == 'struct') {
                         //     echo 'CFN: ', $field_name, ': ', (is_array($variable_row[$field]) ? var_dump($variable_row[$field]) : $variable_row[$field]), '<br />', "\n";
                         // }
                     }
    ...
    {
        $this->foundation->removeEEClass('TMPL');
        $this->foundation->overwriteEEClass('TMPL', $this->original_TMPL);
    }
}
