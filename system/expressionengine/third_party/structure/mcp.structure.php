...
class Structure_mcp
{
	...
	function __construct($switch = TRUE)
	{
		...
		$this->base_url = $this->foundation->getBaseURL();
		...
		if ($this->sql->user_access('perm_admin_channels', $settings)) {
			$nav[lang('cp_channel_settings_title')] = 'channel_settings';
		}

		if ($this->sql->user_access('perm_admin_structure', $settings)) {
			$nav[lang('cp_module_settings_title')] = 'module_settings';
		}
		if ($this->sql->user_access('perm_view_validation', $settings)) {
			$nav[lang('validation')] = 'validation';
		}

		if ($this->sql->user_access('perm_view_nav_history', $settings)) {
			$nav[lang('history')] = 'nav_history';
		}

		// EE2/EE3 handler to create either the top nav (EE2) or side nav (EE3).
		$this->foundation->getNav($nav);
	...
	function index($message = FALSE)
	{
		...
		$data['action_url']            = $this->foundation->moduleURL('delete');
		...
		if ($page_choices && count($page_choices) == 1) {
			$data['add_page_url'] = $this->foundation->cpURL('publish', 'create', array('channel_id'=>key($page_choices)));
		} elseif ($data['page_count'] == 0) {
			$data['add_page_url'] = $this->foundation->moduleURL('channel_settings');
		} else {
			$data['add_page_url'] = '#';
		}
		...
		if (is_array($page_choices) && count($page_choices) > 0) {
			foreach ($page_choices as $key => $channel) {
				$vci++;
				$add_url = (string) $this->foundation->cpURL('publish', 'create', array('channel_id'=>$key, 'template_id'=>$channel['template_id']));
				$add_urls[] = $add_url;
				$add_body .= '<li';
		...
		if(empty($data['data']['page-ui'])) {
			return $this->foundation->view('get_started', $data, TRUE);
		}

		$data['foundation'] = $this->foundation;

		return $this->foundation->view('index', $data, TRUE);
	}
	...
	function channel_settings($message = FALSE)
	{
		...
		$vars['action_url']        = $this->foundation->moduleURL('channel_settings_submit');
		...
		if ($page_choices && count($page_choices) == 1) {
			$vars['add_page_url'] = $this->foundation->cpURL('publish', 'create', array('channel_id'=>key($page_choices)));
		} else {
			$vars['add_page_url'] = '#';
		}
		...
		$vars['foundation'] = $this->foundation;

		return $this->foundation->view('channel_settings', $vars, TRUE);
	}
	...
	function channel_settings_submit()
	{
		...
		foreach($form_data as $key => $data)
		{
			...
			elseif (count($channels) > 0 && array_key_exists($key, $channels))
			{
				ee()->db->where(array('channel_id' => $key, 'site_id' => $this->site_id))->update('structure_channels', $data);
			} else {
		...
		if (count($to_be_deleted) > 0)
		{
			...
			$vars['action_url'] = $this->foundation->moduleURL('delete_channels');
			$vars['attributes'] = array('class' => 'form', 'id' => 'delete_channel');
			$vars['base_url'] = $this->foundation->moduleURL('channel_settings');

			$vars['foundation'] = $this->foundation;

			return $this->foundation->view('delete_channels_confirm', $vars, TRUE);
		}

		$this->foundation->flashData('message_success', "Channel Settings Updated");
		ee()->functions->redirect($this->foundation->moduleURL('channel_settings'));
	}
	...
	function delete_channels()
	{
		...
		$this->foundation->flashData('message_success', "Channels Removed Successfully!");
		ee()->functions->redirect($this->foundation->moduleURL('channel_settings'));
	}
	...
	function module_settings($message = FALSE)
	{
		...
		$vars = array();
		$vars['ee_ver'] = substr(APP_VER, 0, 1);
		$vars['action_url'] = $this->foundation->moduleURL('module_settings_submit');
		...
		return $this->foundation->view('module_settings', $vars, TRUE);
	}
	...
	function module_settings_submit()
	{
		...
		$this->foundation->flashData('message_success', "Structure Settings Updated");
		ee()->functions->redirect($this->foundation->moduleURL('module_settings'));
	}
	...
	function validation()
	{
		...
		if(!$this->sql->user_access('perm_view_validation', $settings))
		{
			ee()->functions->redirect($this->foundation->moduleURL());
		}
		...
		$vars['ee_ver'] = substr(APP_VER, 0, 1);
		$vars['action_url'] = $this->foundation->moduleURL('validation_submit');
		...
		if($vars['total_structure_entries'] == $vars['total_site_pages_entries']) $vars['total_entries_class'] = 'success';
		else $vars['total_entries_class'] = 'error';
		...
		if($vars['ee_orphans'] == 0 && $vars['site_pages_orphans'] == 0 && $vars['structure_orphans'] == 0 && $vars['structure_listing_orphans'] == 0) $vars['total_missing_class'] = 'success';
		else $vars['total_missing_class'] = 'error';
		...
		$vars['listing_id_fix_url'] = $this->foundation->moduleURL('listing_site_id_fix');

		$vars['entries_missing_from_structure'] = $this->entries_missing_from_structure();

		$vars['foundation'] = $this->foundation;
		$vars['themes_dir'] = $this->foundation->getAddonThemesDir();
		...
		//
		// -------------------------------------------
		return $this->foundation->view('validation', $vars, TRUE);
	}
	...
	public function restore()
	{
		...
		if(is_numeric($id))
		{
			...
			if($nav_to_rollback_to->num_rows == 1)
			{
				...
				$this->foundation->flashData('message_success', "Updated!");
				ee()->functions->redirect($this->foundation->moduleURL('nav_history'));
			}
		}
	}
	...
	function validation_submit()
	{
		...
		ee()->functions->redirect($this->foundation->moduleURL('validation'));
	}
	...
	function entries_missing_from_structure()
	{
		...
		foreach($missing_entries as $missing_entry) {
			$missing[] = array(
				'ee_url' => $this->foundation->cpURL('publish', 'edit', array('entry_id'=>$missing_entry->entry_id)),
	...
	{
		if (version_compare('2.6.0', APP_VER, ">")) {
			ee()->cp->cp_page_title = ee()->lang->line($title);
		} else {
	...
	function hook_change_27()
	{
		...
		if ( version_compare('2.7', APP_VER, ">") )
		{
			...
			$this->foundation->flashData('message_success',lang('ee27_hook_complete'));
	...
	{
		// Find out if our sample data channel exists.
