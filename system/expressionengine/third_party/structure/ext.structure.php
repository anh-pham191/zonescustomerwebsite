...
class Structure_ext extends \structure\Eeharbor_ext {
	...
	public function __construct($settings = '')
	{
		...
		if(!$this->foundation->module_installed('structure')) {
			return FALSE;
		}
	...
	public function activate_extension()
	{
		...
		foreach ($hooks as $hook => $method)
		{
			...
			$gte_27 = substr($app_ver, 0, 2) >= "27";

			// Check in place for 2.7+ to install new hooks for safecracker
			if(($hook == "safecracker_submit_entry_end") && $gte_27)
			{
				$hook = 'channel_form_submit_entry_end';
				$method  = 'channel_form_submit_entry_end';
			}

			if(($hook == "channel_module_create_pagination") && $gte_27)
			{
				$hook = 'pagination_create';
			}

			$this->register_extension($method, $hook, $priority);
		}
	...
	public function update_extension($current = false)
	{
		...
		if (version_compare($current, '3.0', "<")) {
			$this->register_extension("channel_module_create_pagination");
			$this->register_extension("wygwam_config");
			$updated = true;
		}

		// add saef hook
		if (version_compare($current, '3.0.5', "<")) {
			$this->register_extension("entry_submission_end");
			$updated = true;
		}

		// add safecracker hook
		if (version_compare($current, '3.1.4', "<")) {
			$this->register_extension("safecracker_submit_entry_end");
			$updated = true;
		}

		// add safecracker hook
		if (version_compare($current, '3.2.4', "<")) {
			$this->register_extension("template_post_parse");
			$updated = true;
		}

		// add core_template_route hook
		if (version_compare($current, '3.2.5', "<")) {
			$this->register_extension("core_template_route");
			$updated = true;
		}

		// add after_channel_entry_save hook
		if (version_compare($current, '4.0.0-a.1', "<")) {
			$this->register_extension("after_channel_entry_save");
		...
		if (version_compare($current, '4.1.0', "<")) {
			$this->register_extension("sessions_end");
			$this->unregister_extension("after_channel_entry_insert");
			$this->unregister_extension("after_channel_entry_update");
			$updated = true;
		}
	...
	function sessions_end($session)
	{
		...
		if(!empty($session->flashdata['structure_redirect']) && $session->flashdata['structure_redirect'] == 1)
		{
			...
			if(!empty($session->flashdata['structure_channel_id'])) $meta['channel_id'] = $session->flashdata['structure_channel_id'];

			$path = 'addons/settings/structure/index';

			$session_type = ee()->config->item('cp_session_type');

			if($session_type == 'cs') $session_id = $session->userdata['fingerprint'];
			elseif($session_type == 's') $session_id = $session->userdata['session_id'];
	...
	public function entry_submission_redirect()
	{
		...
		// Check if EE4 and return the normal URL. Structure will still fire the EE3-style redirect using flashData.
		if($this->foundation->is_ee4()) {
			$entry = $args[0];
		...
		// This hook is used in EE2 and EE4, but not EE3. And the number of parameters have changed.
		// Now we are doing this to get the parameters.
		...
		$cp_call = $args[3];
		$orig_loc = $args[4];
		$return_boolean = (isset($args[5]) ? $args[5] : false);

		return $this->_redirect_url($entry_id, $meta, $data, $cp_call, $orig_loc, $return_boolean);
	}
	...
	private function _redirect_url($entry_id, $meta, $data, $cp_call, $orig_loc, $return_boolean=false)
	{
		...
		if ($cp_call === TRUE && isset($settings['redirect_on_publish']) && $settings['redirect_on_publish'] != 'n')
		{
			...
			if ($settings['redirect_on_publish'] == 'y') {
				...
				else return $this->foundation->moduleURL();
			}
	...
	public function channel_module_create_pagination($ee_obj)
	{
		...
		{
			if ($this->_is_search() === FALSE && isset(ee()->config->_global_vars['structure_pagination_segment']))
			{
				$ee_obj->EE->uri->uri_string = $ee_obj->EE->uri->uri_string . "/P" . ee()->config->_global_vars['structure_pagination_page'];
				$ee_obj->p_page = ee()->config->_global_vars['structure_pagination_page'];
			}
		} else {
	...
	public function channel_form_submit_entry_end($obj)
	{
		...
		if(isset($obj->entry)) {
			if(gettype($obj->entry) == 'object') {
				if(isset($obj->entry->entry_id)) $entry_id = $obj->entry->entry_id;
				if(isset($obj->entry->url_title)) $url_title = $obj->entry->url_title;
			} else {
				if(isset($obj->entry['entry_id'])) $entry_id = $obj->entry['entry_id'];
				if(isset($obj->entry['url_title'])) $url_title = $obj->entry['url_title'];
			}
	...
	private function _create_global_vars()
	{
		...
		// utility global vars
		ee()->config->_global_vars['structure:is:page']           = $this->entry_id !== FALSE && $this->sql->is_listing_entry($this->entry_id) !== TRUE ? TRUE : FALSE;
		...
		ee()->config->_global_vars['structure:page:title']              = $this->entry_id !== FALSE ? $this->sql->get_page_title($this->entry_id) : FALSE; // {page:title}
		ee()->config->_global_vars['structure:page:slug']               = $this->entry_id !== FALSE ? ee()->uri->segment(ee()->uri->total_segments()) : FALSE;
		ee()->config->_global_vars['structure:page:uri']                = $this->entry_id !== FALSE ? $this->uri : FALSE;
		ee()->config->_global_vars['structure:page:url']                = $this->entry_id !== FALSE ? Structure_Helper::remove_double_slashes($this->site_pages['url'] . ee()->config->_global_vars['structure:page:uri']) : FALSE; // {page:url}
