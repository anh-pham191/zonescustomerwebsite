...
<div class="padder ee<?=$ee_ver?> box">
	<div class="tbl-ctrls">
...
<?php 	foreach($orphaned_entries as $entry_id => $status) { ?>
		<tr>
			...
			<td class="<?=($status['ee'] ? 'success' : 'error')?>"><?php if($status['ee'] == 1) echo lang('exists'); else echo lang('missing'); ?></td>
			<td class="<?=($status['site_pages'] ? 'success' : 'error')?>"><?php if($status['site_pages'] == 1) echo lang('exists'); else echo lang('missing'); ?></td>
			<td class="<?=($status['structure'] ? 'success' : 'error')?>"><?php if($status['structure'] == 1) echo lang('exists'); else echo lang('missing'); ?></td>
		</tr>
<?php 	} ?>
		</tbody>
		<tfoot>
