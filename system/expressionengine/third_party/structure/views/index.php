...
<div class="padder ee<?=$ee_ver?> structure-gui">
	<div id="structure-ui" data-ajaxreorder="<?php echo $foundation->moduleURL('ajax_reorder', array('site_id'=>ee()->config->item('site_id'))); ?>" data-ajaxcollapse="<?php echo $foundation->moduleURL('ajax_collapse'); ?>">
		<div id="tree-header"<?php if(count($tabs) >= 8) echo ' class="tree-header-select"'; ?>>
<?php
	if($cp_asset_data OR count($tabs) > 1) {
		if(count($tabs) >= 8) {
			...
			foreach($tabs as $id => $name) {
				echo "\t\t\t\t", '<li', (array_search($id, array_keys($tabs)) == 0 ? ' class="here"' : ''), '><a href="#" rel="', $id, '">', $name, '</a></li>', "\n";
			}
...
<?php
	foreach($tabs as $id => $name) :
		...
		echo "\t\t", '<ul id="', $id, '" class="page-ui page-list', (array_search($id, array_keys($tabs)) > 0 ? ' hide-alt' : ''), '">', "\n";
		...
		foreach ($data[$id] as $eid => $page):
			...
			$add_url = $foundation->cpURL('publish', 'create', array('channel_id'=>$page['channel_id'], 'parent_id'=>$page['parent_id'], 'template_id'=>$tmpl_id));
			$edit_url = $foundation->cpURL('publish', 'edit', array('channel_id'=>$page['channel_id'], 'entry_id'=>$page['entry_id'], 'parent_id'=>$page['parent_id']));
			...
			if (array_key_exists($page['channel_id'], $assigned_channels)) echo '<a href="', $edit_url, '">', (!empty($_GET['debug']) ? $page['entry_id'].': ' : '').$page['title'], '</a>';
			else echo '<span class="page-title-disabled">', $page['title'], '</span>', "\n";

			if ($page['hidden'] == 'y') echo ' <span>(hidden)</span>';

			echo '</span>', "\n";

			// If Listing Exists
			if ($page['listing_cid'] && array_key_exists($page['listing_cid'], $assigned_channels)) {
				echo str_repeat("\t", $indentDepth)."\t\t", '<span class="page-listing"><a href="', $foundation->cpURL('publish', 'create', array('channel_id'=>$page['listing_cid'])), '">', lang('add'), '</a> or <a href="', $foundation->cpURL('listing', 'edit', array('filter_by_channel'=>$page['listing_cid'])), '">', lang('edit'), '</a></span>', "\n";
			}
			...
			if(isset($permissions['view_view_page']) && $permissions['view_view_page'] == 'y') {
				echo str_repeat("\t", $indentDepth)."\t\t\t", '<span class="control-view"><a href="', $foundation->moduleURL('link', array('entry_id'=>$page['entry_id'])), '">', lang('view_page'), '</a></span>', "\n";
			}

			if (isset($permissions['delete']) && ($permissions['delete'] == 'all' || (is_numeric($level_lock_delete)) && ($page['depth'] + 1) > $level_lock_delete)) {
				echo str_repeat("\t", $indentDepth)."\t\t\t", '<span class="control-del"><a href="', $foundation->moduleURL('delete', array('toggle'=>$page['entry_id'])), '">', lang('delete'), '</a></span>', "\n";
			}
	...
	if ($cp_asset_data) {
		...
		foreach ($cp_asset_data as $title => $row):
			if ($row['split_assets'] == 'y') {
				echo "\t\t\t", '<li><span class="listing-title"><a href="', $foundation->cpURL('publish', 'edit', array('channel_id'=>$row['channel_id'], 'entry_id'=>$row['entry_id'])), '">', $row['title'], '</a></span></li>', "\n";
			} else {
				echo "\t\t\t", '<li><span class="listing-title">', $row['title'], '</span><span class="page-listing"><a href="', $foundation->cpURL('publish', 'create', array('channel_id'=>$row['channel_id'])), '">', lang('add'), '</a> or <a href="', $foundation->cpURL('listing', 'edit', array('filter_by_channel'=>$row['channel_id'])), '">', lang('edit'), '</a></span></li>', "\n";
			}
		endforeach;
