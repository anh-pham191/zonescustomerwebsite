...
class Structure_tab
{
	...
	function display($channel_id, $entry_id = '')
	{
	...
	{
		$settings = array();
		if(empty($channel_id)) $channel_id = ee()->input->get_post('channel_id') ? ee()->input->get_post('channel_id') : $this->sql->get_channel_by_entry_id($entry_id);
		...
		if(empty($entry_id)) $entry_id = ee()->input->get_post('entry_id') !== FALSE ? ee()->input->get_post('entry_id') : 0;
		...
		$template_id = array_key_exists($entry_id, $site_pages['uris']) ? $tmpl_id : $structure_channels[$channel_id]['template_id'];
	...
	function validate_publish($params, $channel_entry=null)
	{
		if(isset($channel_entry->channel_id)) $channel_id = $channel_entry->channel_id;
		else $channel_id = $params[0]['channel_id'];

		if(isset($channel_entry->entry_id)) $entry_id = $channel_entry->entry_id;
		elseif(isset($params[0]['entry_id'])) $entry_id = $params[0]['entry_id'];
		elseif(isset($params['entry_id'])) $entry_id = $params['entry_id'];
		else $entry_id = 0;
	...
	function publish_data_db($params, $channel_entry=null)
	{
		if(!empty($_POST['bulk_action'])) return;

		if(isset($channel_entry->entry_id)) $entry_id = $channel_entry->entry_id; // EE3
		elseif(isset($params['entry_id'])) $entry_id = $params['entry_id']; // EE2
		else $entry_id = 0;
		...
		if(isset($channel_entry->channel_id)) $channel_id = $channel_entry->channel_id; // EE3
		elseif(isset($params['meta']['channel_id'])) $channel_id = $params['meta']['channel_id']; // EE2
		else $channel_id = 0;

		if(isset($channel_entry->site_id)) $site_id = $channel_entry->site_id; // EE3
		elseif(isset($params['meta']['site_id'])) $site_id = $params['meta']['site_id']; // EE2
		else $site_id = 0;
		...
		{
			if($params['listing_channel'] == 'n') $listing_channel = 0;
			else $listing_channel = $params['listing_channel']; // EE3
		}
		elseif(isset($params['mod_data']['listing_channel']))
		{
			if($params['mod_data']['listing_channel'] == 'n') $listing_channel = 0;
			else $listing_channel = $params['mod_data']['listing_channel']; // EE2
		}
		else
		{
			// get_listing_channel
		...
		if ($channel_type == 'page' || $channel_type == 'listing')
		{
			...
			if(isset($channel_entry->title)) $title = $channel_entry->title; // EE3
			elseif(isset($params['meta']['title'])) $title = $params['meta']['title']; // EE2
			else $title = '';
			...
			if ($channel_type == 'page')
			{
				...
				{
					$allow_dupes = ee()->extensions->call('structure_allow_dupes', $data['uri']);
				}
	...
	function get_listing_channels($entry_id, $data, $current_channel_id)
	{
		$site_id = ee()->config->item('site_id');
		...
		if ($count_listings > 0)
		{
			...
			{
				if($channel_id == $current_channel_id) continue;
	...
	function create_uri($str)
	{
		if($this->foundation->is_ee4()) {
			$uri = ee('Format')->make('Text', $str)->urlSlug();
		} else {
			$uri = url_title($str, ee()->config->item('word_separator'), TRUE);
		}
