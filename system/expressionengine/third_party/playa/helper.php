...
/**
 * Playa Helper class for ExpressionEngine 2 & 3
 *
 * @package		Playa
...
class Playa_Helper {
	...
	function param2sql($param)
	{
		...
		if (is_string($param))
		{
			...
			{
				$param = ee()->session->userdata('member_id');
			}
	...
	function db_where($col, $val)
	{
		...
		elseif (count($val) == 1)
		{
			ee()->db->where($col, $val[0]);
		}
		else
		{
			ee()->db->where_in($col, $val);
		}
	}
	...
	function entries_query($params)
	{
		...
		$where = array();
			...
			{
				$where[] = 'ct.entry_date >= '.ee()->localize->convert_human_date_to_gmt($params['start_on']);
			}

			if (isset($params['stop_before']) && $params['stop_before'])
			{
				$where[] = 'ct.entry_date < '.ee()->localize->convert_human_date_to_gmt($params['stop_before']);
			}

			$timestamp = (isset(ee()->TMPL) && ee()->TMPL->cache_timestamp) ? ee()->TMPL->cache_timestamp : ee()->localize->now;
			...
			{
				if (ee()->session->userdata('can_edit_other_entries') == "n")
				{
					$user = (int) ee()->session->userdata('member_id');
					$where[] = 'ct.author_id = '.$user;
				}
			}
			...
			{
				$params['keywords'] = ee()->db->escape_like_str($params['keywords']);
		...
		// ----------------------------------------------
		// Make sure that distinct entry IDs are returned
		// ----------------------------------------------
		if (empty($params['count']))
		{
		...
		if (isset($params['orderby']) && $params['orderby'])
		{
			...
			foreach ($orderbys as $i => $attr)
			{
				...
				$attr = $attr == 'date' ? 'entry_date' : $attr;
	...
	function _escape_variable($variable)
	{
		...
		{
			$variable = ee()->db->escape_str($variable);
		}
	...
	function create_module_tag($params, $tagdata, $func)
	{
		...
		{
			$var_prefix = 'playa'.ee()->functions->random('alnum', 8);
