...
class FF2EE2 {
	 ...
	 * Constructor
	 */
	public function __construct($class, $field_settings_callback = FALSE)
	{
		...
		if (! ee()->db->table_exists('ff_fieldtypes'))
		{
			return;
		}
		...
		$ftype = ee()->db->select('fieldtype_id, version, settings')
		...
		$fields = ee()->db->where('field_type', 'ftype_id_'.$this->fieldtype_id)->get('channel_fields');
		...
		foreach($fields->result_array() as $field)
		{
			...
			ee()->db->where('field_id', $field_id)->update('channel_fields', array(
