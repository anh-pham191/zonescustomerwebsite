...
/**
 * Playa Fieldtype Class for ExpressionEngine 2 & 3
 *
 * @package   Playa
...
class Playa_ft extends EE_Fieldtype {
	...
	var $is_element; // True if this is a Content Element
	...
	function __construct()
	{
		...
		if (! ee()->db->table_exists('playa_relationships'))
		{
			...
			$query = ee()->db->select('fieldtype_id, version')
			                      ->where('name', 'playa')
			                      ->get('fieldtypes');
			...
			if ($query->num_rows())
			{
				...
				ee()->db->where('fieldtype_id', $query->row('fieldtype_id'))
	...
	function install()
	{
		...
		else
		{
			...
			$query = ee()->db->select('field_id, field_list_items')
	...
	private function _create_table()
	{
		if (! ee()->db->table_exists('playa_relationships'))
		{
			...
			ee()->dbforge->add_field(array(
				...
				'parent_element_id'   => array('type' => 'varchar', 'constraint' => 255, 'null' => TRUE),
			...
			ee()->dbforge->add_key('parent_entry_id');
			ee()->dbforge->add_key('parent_field_id');
			ee()->dbforge->add_key('parent_col_id');
			ee()->dbforge->add_key('parent_row_id');
			ee()->dbforge->add_key('parent_var_id');
			ee()->dbforge->add_key('child_entry_id');

			ee()->dbforge->create_table('playa_relationships');

			// add it to the table names cache
			ee()->db->data_cache['table_names'][] = ee()->db->dbprefix.'playa_relationships';
	...
	function update($from)
	{
		if ($from)
		{
			if (version_compare($from, '4.0', '<'))
			{
				if ($this->_create_table())
				{
					...
					foreach ($fields->result() as $field)
					{
						...
						// This Channel Data query is for an old version, so it can stay
						...
						foreach ($entries->result() as $entry)
						{
							...
							if ($rel_ids)
							{
								...
								if (version_compare(APP_VER, '2.6', '<'))
								{
									...
									$rels = ee()->db->select('rel_id, rel_child_id')
								...
								{
									$entries = ee()->db->select('relationship_id, parent_id, child_id, field_id, order')
					...
					if (ee()->db->table_exists('matrix_cols'))
					{
						...
						foreach ($cols->result() as $col)
						{
							...
							$rows = ee()->db->select('row_id, entry_id, field_id, '.$col_id)
					...
					if (!empty($rel_data))
					{
						ee()->db->insert_batch('playa_relationships', $rel_data);
					}
					...
					if (!empty($old_rel_ids))
					{
						...
						if (version_compare(APP_VER, '2.6', '<'))
						{
							ee()->db->where_in('rel_id', $old_rel_ids)
								->delete('relationships');
						}
						else
						{
							ee()->db->where_in('relationship_id', $old_rel_ids)
								->delete('relationships');
						}
					}
				}
			...
			{
				// Clear the table name cache. If the tables names have been updated on this runtime, the cache doesnt know.
				unset( ee()->db->data_cache );

				if (!ee()->db->field_exists('parent_is_draft', 'playa_relationships'))
				{
					// Just add the var_id column
					ee()->db->query('ALTER TABLE exp_playa_relationships ADD parent_var_id INT(6) UNSIGNED AFTER parent_row_id, ADD INDEX (parent_var_id)');
				}
			}
			...
			if (version_compare($from, '4.3.1', '<'))
			{
				if (ee()->db->table_exists('matrix_data'))
				{
					...
					ee()->db->query('DELETE rel FROM exp_playa_relationships AS rel
			...
			if (version_compare($from, '4.4', '<'))
			{
				...
				if (!ee()->db->field_exists('parent_is_draft', 'playa_relationships'))
				{
					...
					ee()->db->query('ALTER TABLE exp_playa_relationships ADD parent_is_draft INT(1) UNSIGNED NOT NULL DEFAULT 0 AFTER parent_var_id');
				}
			}
			...
			{
				$result = ee()->db->query("SHOW FIELDS FROM exp_playa_relationships")->result();
				$has_column = FALSE;
			...
			if (version_compare($from, '4.4.4', '<'))
			{
				...
				{
					ee()->db->query('ALTER TABLE exp_playa_relationships MODIFY parent_is_draft INT(1) UNSIGNED NOT NULL DEFAULT 0');
				}
				ee()->db->query('UPDATE exp_playa_relationships SET parent_is_draft = 0 WHERE parent_is_draft <> 1');
			}

			if (version_compare($from, '4.5', '<'))
			{
				// Clear the table name cache. If the tables names have been updated on this runtime, the cache doesnt know.
				unset( ee()->db->data_cache );

				if (!ee()->db->field_exists('parent_element_id', 'playa_relationships'))
				{
					ee()->db->query('ALTER TABLE exp_playa_relationships ADD parent_element_id VARCHAR(255) NULL AFTER parent_var_id');
				}
			}

			// update the module version number
			ee()->db->where('module_name', 'Playa')
	...
	{
		ee()->cp->add_to_head('<link rel="stylesheet" type="text/css" href="'.$this->_theme_url().$file.'" />');
	}
	...
	{
		ee()->cp->add_to_foot('<script type="text/javascript" src="'.$this->_theme_url().$file.'"></script>');
	}
	...
	function display_global_settings()
	{
		...
		$query = ee()->db->query('SELECT cf.field_id, cf.field_label, fg.group_name
		...
		if (ee()->db->table_exists('related_entries'))
		{
	...
	function save_global_settings()
	{
		...
		if ($fields = ee()->input->post('convert_rel_field'))
		{
			$field_ids = array();
			...
			if ($field_ids)
			{
				...
				foreach ($fields->result() as $field)
				{
					...
					if ($field->field_type == 'mrel')
					{
						...
						// *** Our global settings do no work in ee3-ee4. I think due to this, we can just ignore this query,
						// which would result in an error in ee4.
						$entries = ee()->db->select("entry_id, {$field_name}")
						                        ->where("{$field_name} !=", '')
						                        ->get('channel_data');
						...
						if ($rel_ids)
						{
							...
							{
								// get the old relationships
								$rels = ee()->db->select('rel_id, rel_parent_id, rel_child_id')
													 ->where_in('rel_id', $rel_ids)
													 ->get('relationships');
							}
							else
							{
								// get the old relationships
								$rels = ee()->db->select('relationship_id, parent_id, child_id')
									->where_in('relationship_id', $rel_ids)
									->get('relationships');
							}
						}
					...
					else
					{
						...
						{
							// This Channel Data query is for an old version, so it can stay!
							$rels = ee()->db->query('SELECT r.rel_id, r.rel_parent_id, r.rel_child_id
						                              FROM exp_relationships AS r
						                              INNER JOIN exp_channel_data AS cd ON cd.'.$field_name.' = r.rel_id');
						}
						else
						{
							$rels = ee()->db->query('SELECT r.relationship_id, r.parent_id, r.child_id
						                              FROM exp_relationships AS r
		...
		if (ee()->db->table_exists('related_entries') && ee()->input->post('convert_related_entries') == 'y')
		{
			// get the field groups
			$query = ee()->db->query('SELECT fg.group_id, fg.site_id, fg.group_name,
			...
			if ($query->num_rows())
			{
				...
				$query = ee()->db->query('SELECT re.entry_id, re.related_entry_id, re.main, c.field_group
				...
				foreach ($query->result() as $rel)
				{
					...
					if (! isset($group['playa_field_id']))
					{
						...
						for ($i = 0; $i < 50; $i++)
						{
							...
							$count = ee()->db->select('COUNT(*) AS count')
						...
						ee()->db->insert('channel_fields', $data);

						// get the new field_id
						$field_id = ee()->db->insert_id();
						...
						// This Channel Data query is for an old version, so it can stay
						ee()->dbforge->add_column('channel_data', $fields);
		...
		return array(
			...
			'filter_min' => ee()->input->post('filter_min')
		);
	}
	...
	function display_element_settings($data)
	{
		...
		// needs additional proccessing for ee3
		return $this->_get_field_setting_rows($rows);
	}
	...
	private function _field_settings($data, $is_cell = FALSE)
	{
		...
		$ft_data[] = array(
			'title' => lang('allow_multiple_selections', 'playa_multi'),
			'desc' => ($is_cell ? '' : lang('multi_info')),
			'fields' => array(
	            'playa[multi]' => array(
	                'type' => "html",
	                'content' => form_radio('playa[multi]', 'y', ($data['multi'] == 'y'), 'id="playa_multi_y"') . NL
		...
		$ft_data[] = array(
			'title' => lang('show_expired_entries', 'playa_expired'),
			'desc' => '',
			'fields' => array(
	            'playa[expired]' => array(
	                'type' => "html",
	                'content' => form_radio('playa[expired]', 'y', ($data['expired'] == 'y'), 'id="playa_expired_y"') . NL
		...
		// Show future entries?
		$ft_data[] = array(
			'title' => lang('show_future_entries', 'playa_future'),
			'desc' => '',
			'fields' => array(
	            'playa[future]' => array(
	                'type' => "html",
	                'content' => form_radio('playa[future]', 'y', ($data['future'] == 'y'), 'id="playa_future_y"') . NL
		...
		$ft_data[] = array(
			'title' => lang('only_show_editable_entries', 'playa_editable'),
			'desc' => ($is_cell ? '' : lang('only_show_editable_entries_info')),
			'fields' => array(
	            'playa[editable]' => array(
	                'type' => "html",
	                'content' => form_radio('playa[editable]', 'y', ($data['editable'] == 'y'), 'id="playa_editable_y"') . NL
		...
		{
			$ft_data[] = array(
				'title' => lang('sites', 'playa_sites'),
				'desc' => '',
				'fields' => array(
		            "playa['sites'][]" => array(
		                'type' => "html",
		                'content' => $this->_sites_select($data['sites'])
		            )
		        )
			);
		}
		...
		// Channels
		$ft_data[] = array(
			'title' => lang('channels', 'playa_channels'),
			'desc' => '',
			'fields' => array(
		        "playa['channels'][]" => array(
	                'type' => "html",
	                'content' => $this->_channels_select($data['channels'])
	            )
	        )
		);
		...
		// Categories
		$ft_data[] = array(
			'title' => lang('cats', 'playa_cats'),
			'desc' => '',
			'fields' => array(
		        "playa['cats'][]" => array(
	                'type' => "html",
	                'content' => $this->_cats_select($data['cats'])
	            )
	        )
		);
		...
		// Member groups
		$ft_data[] = array(
			'title' => lang('member_groups', 'playa_member_groups'),
			'desc' => '',
			'fields' => array(
		        "playa['member_groups'][]" => array(
	                'type' => "html",
	                'content' => $this->_member_groups_select($data['member_groups'])
	            )
	        )
		);
		...
		// Authors
		$ft_data[] = array(
			'title' => lang('authors', 'playa_authors'),
			'desc' => '',
			'fields' => array(
		        "playa['authors'][]" => array(
	                'type' => "html",
	                'content' => $this->_authors_select($data['authors'])
	            )
	        )
		);
		...
		// Statuses
		$ft_data[] = array(
			'title' => lang('statuses', 'playa_statuses'),
			'desc' => '',
			'fields' => array(
		        "playa['statuses'][]" => array(
	                'type' => "html",
	                'content' => $this->_statuses_select($data['statuses'])
	            )
	        )
		);
		...
		// Limit entries to
		$ft_data[] = array(
			'title' => lang('limit_entries_to', 'playa_limit'),
			'desc' => '',
			'fields' => array(
		        "playa['limit']" => array(
	                'type' => "html",
	                'content' => '<select name="playa[limit]" onchange="$(this).next().css( \'visibility\', this.value==\'0\'?\'hidden\':\'visible\')">'
									.   $this->_select_options($data['limit'], $limit_options)
									. '</select>'
	            ),
		        "playa['limitby']" => array(
	                'type' => "html",
	                'content' => '<select name="playa[limitby]" style="margin-left: 4px;'.($data['limitby'] ? '' : ' visibility:hidden;').'">'
		...
		// Order entries by
		$ft_data[] = array(
			'title' => lang('order_entries_by', 'playa_order'),
			'desc' => '',
			'fields' => array(
		        "playa['orderby']" => array(
	                'type' => "html",
	                'content' => '<select name="playa[orderby]">'
		        ...
		        "playa['sort']" => array(
	                'type' => "html",
	                'content' => '<select name="playa[sort]">'
		...
		$playa_data = array('Playa' => array(
		        'label' => 'field_options',
		        'group' => 'playa',
		        'settings' => $ft_data
		    ));

		// if not ee2, the data is already ready!
		if( ! $this->foundation->is_ee2())
		{
			return $playa_data;
		}

		// this is the EE2 return data
		return $this->_get_field_setting_rows($playa_data, $is_cell);
	}

	// --------------------------------------------------------------------

	private function _get_field_setting_rows($ft_data, $is_cell = FALSE)
	{
		$r = array();
		foreach($ft_data['Playa']['settings'] as $ft_row)
		{
			$row = array();
			$row[] = $ft_row['title'] . ($is_cell ? '' : '<br />' . $ft_row['desc']);

			$content = '';
			foreach($ft_row['fields'] as $fieldname => $info)
			{
	...
	{
		$sites = ee()->db->query('SELECT site_id AS `id`, site_label AS `title`
		                               FROM exp_sites
		                               ORDER BY site_label')
		                       ->result_array();
	...
	private function _channels_select($selected_channels)
	{
		...
		$channels = ee()->db->query('SELECT c.channel_id AS `id`, c.channel_title AS `title`, s.site_label AS `group`
	...
	private function _cats_select($selected_cats)
	{
		...
		$cats = ee()->db->query('SELECT c.cat_id AS `id`, c.cat_name AS `title`, c.parent_id, cg.group_name AS `group`
	...
	private function _authors_select($selected_authors)
	{
		...
		$publish_name = $this->get_publish_column_name();

		$authors = ee()->db->query('SELECT m.member_id AS `id`, m.screen_name AS `title`, mg.group_title AS `group`
		                                 FROM exp_members m, exp_member_groups mg
		                                 WHERE m.group_id = mg.group_id
		                                       AND mg.'.$publish_name.' = "y"
	...
	private function _member_groups_select($selected_groups)
	{
		...
		$groups = ee()->db->query('SELECT DISTINCT(mg.group_id) AS `id`, mg.group_title AS `title`
		                                 FROM exp_member_groups AS mg
		                                 WHERE mg.'.$publish_name.' = "y"
		                                       '.($this->msm ? '' : 'AND mg.site_id = "'.$site_id.'"').'
	...
	private function _statuses_select($selected_statuses)
	{
		...
		// The exp_statuses table has changes in EE4
		// Also the exp_status_groups table is just gone all together
		if($this->foundation->ver_lt(4))
		{
			$statuses = ee()->db->query('SELECT s.status AS `id`, s.status AS `title`, sg.group_name AS `group`
		...
		{
			$statuses = ee()->db->query('SELECT status, status AS `title`, status AS `id` FROM exp_statuses
	 ...
	 * Display Field
	 */
	function display_field($data, $content_id=0)
	{
		if(empty($content_id)) {
			if(method_exists($this, 'content_id')) {
				$content_id = $this->content_id();
			} else {
				$post_content_id = ee()->input->get_post('entry_id');

				if(!empty($post_content_id)) {
					$content_id = ee()->input->get_post('entry_id');
				}
			}
		}
		...
		if (! isset($this->cache['included_shared_resources']))
		{
			...
			$statuses = ee()->db->query('SELECT status, highlight FROM exp_statuses
			                                  WHERE status NOT IN ("open", "closed")');
			...
			$query = ee()->db->query('SELECT action_id FROM exp_actions WHERE class = "Playa_mcp" AND method = "filter_entries"');
			...
			if ($this->cache['filter_action_id'])
			{
				...
				if (($site_index = ee()->config->item('playa_site_index')) === FALSE) $site_index = ee()->functions->fetch_site_index(0, 0);
			...
			// View path
			ee()->load->add_package_path(PATH_THIRD.'playa');
		}

		// is this a Matrix cell?
		$is_cell = isset($this->cell_name);

		if (! $this->var_id && !$this->element_id)
		{
		...
		else
		{
			...
			if (($this->var_id || !empty($entry_id) || $this->element_id || $content_id) && (! $is_cell || $this->row_id))
			{
				if ($this->var_id)
				{
					$where = array(
				...
				//
					if (ee()->extensions->active_hook('playa_field_selections_query'))
					{
						$rels = ee()->extensions->call('playa_field_selections_query', $this, $where);
					}
					else
					{
		...
		if ($vars['selected_entry_ids']
			&& ($query = ee()->db->query('SELECT * FROM exp_channel_titles
		...
		if ($this->settings['editable'] == 'y')
		{
			...
			if ($group_id != 1)
			{
				$query = ee()->db->query('SELECT channel_id FROM exp_channel_member_groups
		...
		if (($key = array_search('current', $this->settings['member_groups'])) !== FALSE)
		{
			...
			if (ee()->session->userdata('group_id') == 1 && ee()->config->item('playa_sa_all_authors'))
			{
		...
		$cache_key = $this->settings['expired']
		           ...
		           . $flat_authors       . ','
		           . $flat_statuses . ','
			       . (isset($this->settings['sites']) ? implode("-", $this->settings['sites']) : "");
		...
		return $this->$func($data, $vars, $opts);
	}

	/**
	 * Display Cell
	 */
	function display_cell($data, $content_id=0)
	{
		return $this->display_field($data, $content_id);
	}
	...
	function display_element($data)
	{
		...
		if (! isset($this->cache['included_content_resources']))
		{
			...
			$this->_insert_css('.content_elements_icon_playa { background: url('.$this->_theme_url().'images/ce_icon.png); background-size: 16px; }');

			// For some reason, if Playa field is loaded before a CE field, this will be unloaded.
			ee()->load->add_package_path(PATH_THIRD.'playa');
			$this->_include_theme_js('scripts/content_elements.js');
			$this->cache['included_content_resources'] = TRUE;
		}

		$query = $query = ee()->db->select('settings')->where('name', 'playa')->get('fieldtypes');
	 ...
	 */
	private function _display_field_droppanes($data, $vars, $opts)
	{
		$is_cell = isset($this->cell_name);
		...
		{
			if ((!$is_cell && !$is_element) || ($is_cell && !isset($this->cache['initialized_cols'][$this->col_id])) || $is_element)
			{
				// MSM?
				...
				if($this->foundation->ver_lt(4))
				{
					$channels = ee()->db->query('SELECT c.channel_id AS `id`, c.channel_title AS `title`, c.cat_group, c.status_group, s.site_label AS `group`
					                                  FROM exp_channels c, exp_sites s
					                                  WHERE s.site_id = c.site_id
					...
					$channels = ee()->db->query('SELECT c.channel_id AS `id`, c.channel_title AS `title`, c.cat_group, s.site_label AS `group`
					                                  FROM exp_channels c, exp_sites s
					                                  WHERE s.site_id = c.site_id
					                                        '.($msm ? '' : 'AND s.site_id = "'.$site_id.'"').'
					                                        '.($this->settings['channels'] ? 'AND c.channel_id '.$this->helper->param2sql($this->settings['channels']) : '').'
				...
				foreach ($channels as &$channel)
				{
					...
					if (isset($channel['status_group']) && $channel['status_group']) $status_groups = array_merge($status_groups, array($channel['status_group']));
				...
				if ($cat_groups)
				{
					...
					if ($cats->num_rows())
					{
						...
						// If categories are set, include only the selected categories for filters
						if (isset($this->settings['cats']) && is_array($this->settings['cats']) && count($this->settings['cats']) > 0)
						{
							$new_cats_options = array();
							foreach ($cats_options as $option)
							{
								if (in_array($option['id'], $this->settings['cats']))
								{
									$new_cats_options[] = $option;
								}
							}
							$cats_options = $new_cats_options;
						}
		...
		if (! ($is_cell || $is_element))
		{
			$this->_insert_js('new PlayaDropPanes(jQuery("#'.$vars['field_id'].'")'.($opts_json ? ', '.$opts_json : '').');');
		}

		if ($this->is_element && $this->element_id)
		{
			$field_id = rtrim(preg_replace('/(\[|\])+/', '_', $this->field_name), '_');
			$this->_insert_js('new PlayaDropPanes(jQuery("#'.$field_id.'")'.($opts_json ? ', '.$opts_json : '').');');
		}
		...
		$r = ee()->load->view('droppanes', $vars, TRUE);
	 ...
	 */
	private function _display_field_singleselect($data, $vars, $opts)
	{
		$is_cell = isset($this->cell_name);
		...
		$r = ee()->load->view('singleselect', $vars, TRUE);
		return $this->helper->strip_whitespace($r);
	}
	...
	function save_element($data)
	{
		...
		$this->cache['selections'][$this->element_id] = $selections;
	...
	public function draft_discard()
	{
		...
		ee()->db->where($where)->delete('playa_relationships');
	}
	...
	public function draft_publish()
	{
		...
		ee()->db->where($where)->update('playa_relationships', $update);
	...
	/**
	 * Discard a BWF+CE draft.
	 */
	public function discard_element_draft()
	{
		$where = array(
			'parent_element_id' => $this->element_id,
			'parent_is_draft' => 1
		);
	...
	public function save_element_draft($data)
	{
		...
		if (! isset($this->cache['selections'][$this->element_id])) return $data;
	...
	function post_save($data)
	{
		...
		if(!empty($this->settings['entry_id'])) $entry_id = $this->settings['entry_id'];
		elseif(!empty($this->entry_id)) $entry_id = $this->entry_id;
		elseif(!empty($content_id)) $entry_id = $content_id;
	...
	{
		// make sure this should have been called in the first place
		if (! isset($this->cache['selections'][$this->element_id])) return;

		// get the selections from the cache
		$selections = $this->cache['selections'][$this->element_id];
	...
	function settings_modify_column($data)
	{
		if ($data['ee_action'] == 'delete')
		{
			...
			ee()->db->where('parent_field_id', $data['field_id'])
			             ->delete('playa_relationships');
		}
	...
	function settings_modify_matrix_column($data)
	{
		if ($data['matrix_action'] == 'delete')
		{
			...
			ee()->db->where('parent_col_id', $data['col_id'])
			             ->delete('playa_relationships');
		}
	}
	...
	function delete_rows($row_ids)
	{
		...
		ee()->db->where_in('parent_row_id', $row_ids)
		             ->delete('playa_relationships');
	}
	...
	{
		ee()->db->where('parent_var_id', $var_id)
		             ->delete('playa_relationships');
	}
	...
	function replace_element_tag($data, $params = array(), $tagdata = FALSE)
	{
		if ($data)
		{
			if (preg_match_all('/(\{children(\s.*?)?\}(.*?)\{\/children\})/s', $tagdata, $matches))
			{
			...
			return ee()->functions->var_swap($tagdata, array(
	...
	/**
	 * [get_publish_column_name for ee2/ee3 compatibility]
	 * @return [string] [description]
	 */
	private function get_publish_column_name()
	{
		if($this->foundation->is_ee2()) {
			return "can_access_publish";
		} else {
