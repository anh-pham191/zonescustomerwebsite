...
/**
 * Playa Module Class for ExpressionEngine 2 & 3
 *
 * @package		Playa
...
class Playa
{
	...
	function __construct()
	{
		...
		$entry_ids = ee()->TMPL->fetch_param('entry_id');
	...
	{
		$tagdata = (string)ee()->TMPL->tagdata;
		$var_prefix = rtrim(ee()->TMPL->fetch_param('var_prefix'), ':');
	...
	private function _names_to_ids($what, $table, $names, $site_id)
	{
		...
		if ($get_names)
		{
			ee()->db->select($what.'_id AS id, '.$what.'_name AS name');
	...
	{
		if ($var_id = ee()->TMPL->fetch_param('var_id'))
		{
			// unset the param
			unset(ee()->TMPL->tagparams['var_id']);

			return explode('|', $var_id);
		}

		if (($var_names = ee()->TMPL->fetch_param('var')))
		{
			// unset the param
			unset(ee()->TMPL->tagparams['var']);
	 ...
	 * Fetch Element IDs
	 *
	 * Returns an array of element IDs associated with this tag,
	 * whether passed via the element_id=
	 */
	private function _fetch_element_ids()
	{
		if ($element_id = ee()->TMPL->fetch_param('element_id'))
		{
			// unset the param
			unset(ee()->TMPL->tagparams['element_id']);

			return explode('|', $element_id);
		}
	}
	...
	private function _fetch_field_ids()
	{
		...
		{
			// unset the param
			unset(ee()->TMPL->tagparams['field_id']);

			return explode('|', $field_id);
		}

		if (($field_names = ee()->TMPL->fetch_param('field')))
		{
			// unset the param
			unset(ee()->TMPL->tagparams['field']);
	...
	{
		if ($col_id = ee()->TMPL->fetch_param('col_id'))
		{
			// unset the param
			unset(ee()->TMPL->tagparams['col_id']);

			return explode('|', $col_id);
		}

		if ($col_names = ee()->TMPL->fetch_param('col'))
		{
			// unset the param
			unset(ee()->TMPL->tagparams['col']);
	...
	{
		if ($row_id = ee()->TMPL->fetch_param('row_id'))
		{
			// unset the param
			unset(ee()->TMPL->tagparams['row_id']);
	...
	{
		ee()->TMPL->no_results_block = $m[0];
		ee()->TMPL->no_results = $m[1];
	...
	private function _fetch_entry_row($entry_id)
	{
		if (! isset($this->cache['entry_rows'][$entry_id]))
		{
			...
			$row = ee()->db->query("SELECT ct.author_id, c.channel_title, c.channel_id, c.channel_name, ct.entry_date, ct.site_id AS entry_site_id, ct.expiration_date, m.screen_name, ct.status, ct.title, ct.url_title
	...
	private function _parse_relative_entry_tags(&$tagdata, $tag_prefix)
	{
		...
		$tagdata = ee()->TMPL->parse_variables_row($tagdata, $rel_row_tags);
	}
	...
	private function _parse_relationships($tagdata, $entry_ids, $fixed_order)
	{
		...
		$entry_id_param = ($fixed_order && ! ee()->TMPL->fetch_param('orderby')) ? 'fixed_order' : 'entry_id';
		ee()->TMPL->tagparams[$entry_id_param] = '0|'.$entry_ids;
		...
		//
			if (ee()->extensions->active_hook('playa_parse_relationships'))
			{
				ee()->extensions->call('playa_parse_relationships');
			}
		//
		// -------------------------------------------
	...
	private function _fetch_rel_entry_ids($dir)
	{
		...
		$element_ids = $this->_fetch_element_ids();
		...
		$filter_ids = array(
			...
			'parent' => ee()->TMPL->fetch_param('parent_id')
		);
		...
		if (! isset($this->cache['rels'][$cache_key]))
		{
			...
			switch($dir)
			{
				case 'children':
					...
					$sql = 'SELECT DISTINCT(rel.child_entry_id) AS entry_id,
							rel.parent_entry_id, rel.parent_field_id, rel.parent_row_id, rel.parent_col_id, rel.parent_var_id, rel.rel_order
				...
				case 'parents':
					...
					$sql = 'SELECT DISTINCT(rel.parent_entry_id) AS entry_id,
							rel.parent_field_id, rel.parent_row_id, rel.parent_col_id, rel.parent_var_id, rel.rel_order
					        FROM exp_playa_relationships rel';
			...
			if ($var_ids)
			{
				if (count($var_ids) == 1)
				{
					$where[] = 'rel.parent_var_id = '.ee()->db->escape_str($var_ids[0]);
				}
				else
				{
			...
			if ($element_ids)
			{
				...
				{
					$where[] = 'rel.parent_element_id = "'.ee()->db->escape_str($element_ids[0]).'"';
				}
				else
				{
					$where[] = 'rel.parent_element_id IN ('.implode(',', $element_ids).')';
				}
			}
			// filter by field?
			if ($field_ids)
			...
			//
				if (ee()->extensions->active_hook('playa_fetch_rels_query'))
				{
					$data = array(
						'and' => $this->and,
						'dir' => $dir,
					...
					$rels = ee()->extensions->call('playa_fetch_rels_query', $this, $sql, $data);
				}
				else
				{
					$rels = ee()->db->query($sql);
				}
			//
			// -------------------------------------------
	...
	private function _fetch_rel_entry_ids_str($dir, $ids_tag = FALSE)
	{
		...
		// get the delimiter
		$delimiter = ($ids_tag && ($delimiter = ee()->TMPL->fetch_param('delimiter')) !== FALSE) ? $delimiter : '|';

		// flatten the array
		$r = implode($delimiter, $entry_ids);

		// backspace=
		if ($ids_tag && ($backspace = ee()->TMPL->fetch_param('backspace')))
		{
	...
	{
		if ($sibling_ids = ee()->TMPL->fetch_param('sibling_id'))
		{
			ee()->TMPL->tagparams['child_id'] = $sibling_ids;
		}
	...
	{
		if ($coparent_ids = ee()->TMPL->fetch_param('coparent_id'))
		{
			ee()->TMPL->tagparams['parent_id'] = $coparent_ids;
		}
